...
class EpisodeRepository @Inject constructor(
    ...
    private val cache = TimedCachedResource<String, List<TraktSeasonWithEpisodes>>(
        ...
        fetcher = { showId -> fetchFromProvider(showId) },
    )
    ...
    suspend fun markEpisodeWatched(
        ...
        trackingProvider.markWatched("Bearer $token", ids, seasons).getOrThrow()
    ...
    suspend fun markEpisodeUnwatched(
        ...
        val token = tokenRefreshManager.getValidAccessToken()
            ?: error("No access token available")
        trackingProvider.markUnwatched("Bearer $token", ids, season, episode).getOrThrow()
        invalidateShowCache(ids)
        Unit
    }.onFailure { Log.w(TAG, "markEpisodeUnwatched S${season}E${episode} failed", it) }

    /**
     * Marks all [candidates] (season, episode) pairs as watched in a single call.
     * The caller is responsible for pre-filtering the candidate list (season >= 1,
     * not already watched, <= target position).
     *
    ...
    suspend fun markEpisodesWatchedUpTo(
        ...
        val token = tokenRefreshManager.getValidAccessToken()
            ?: error("No access token available")
        trackingProvider.markWatched("Bearer $token", ids, seasonsBody).getOrThrow()
        invalidateShowCache(ids)
        Unit
    }.onFailure {
    ...
    private suspend fun fetchFromProvider(showId: String): List<TraktSeasonWithEpisodes> {
        val token = tokenRefreshManager.getValidAccessToken()
            ?: error("No access token available")
        return trackingProvider.getSeasonsWithEpisodes("Bearer $token", showId)
    }

    private suspend fun invalidateShowCache(ids: TraktIds) {
        val key = ids.trakt?.toString() ?: ids.simkl?.toString() ?: ids.slug ?: return
