...
- **TV discovery lifecycle & boot autostart:** Discovery is user-controlled via the `isPhoneDiscoveryEnabled` toggle in `StreamingPreferencesRepository` (default on). `TvHomeViewModel` observes the flow and drives `PhoneDiscoveryManager.setEnabled(...)` — discovery outlives `TvMainActivity` recreation. A separate `isAutostartEnabled` toggle (default off) lets `BootReceiver` start the foreground `TvDiscoveryService` on `BOOT_COMPLETED`. See [`docs/architecture.md` § Communication Protocol](docs/architecture.md#communication-protocol-tv--phone) for the full lifecycle (FGS type, permissions, Hilt wiring via `EntryPointAccessors`). **Idle timeout (#540):** `TvDiscoveryService` self-stops gracefully (logging a `DiagnosticLog.event`) to avoid draining the Android 14+ 24-hour FGS quota: (1) after 1 h if zero phones were ever discovered (`IdleTimeoutMonitor.Reason.NO_DISCOVERY`), (2) after 30 min of all previously-discovered phones being absent (`IdleTimeoutMonitor.Reason.ALL_UNREACHABLE`). Logic lives in `discovery/IdleTimeoutMonitor.kt` (pure coroutines, fully JVM-testable); constants are `DiscoveryConstants.NO_DISCOVERY_TIMEOUT_MS` / `ALL_UNREACHABLE_TIMEOUT_MS`. Toggling discovery off/on resets both clocks.
...
- **Scrobbling:** `MediaSessionScrobbler` (core, consumed by `TvScrobbleDispatcher`) listens to active media sessions on the TV, builds a `MediaMetadataSnapshot` by first collecting standard MediaMetadata fields (`TITLE`, `DISPLAY_TITLE`, `DISPLAY_SUBTITLE`, `DISPLAY_DESCRIPTION`, `ARTIST`, `ALBUM_ARTIST`, `ALBUM`, `SUBTITLE`) then calling each registered `MetadataEnricher` in sequence. The TV registers two enrichers (via `AppModule.provideMetadataEnrichers`): (a) `WatchNextMetadataSource` — queries the Android TV WatchNext content provider for `watchNext.*` tagged lines; (b) `NotificationMetadataSource` — reads cached MediaStyle notification extras (`notification.title`, `notification.text`, `notification.subText`, `notification.bigText`, `notification.infoText`) stored by `WatchBuddyNotificationListener.onNotificationPosted`. Both enrichers apply a two-layer freshness gate (`PlaybackTick.isPlaying` + a 5-min snippet-age cap). Before the match cascade, `AppProfiles.forPackage(packageName)` looks up a static per-app profile (`core/scrobbler/AppProfiles.kt`) that can reorder evidence lines (`preferredSourceTags`), supply extra episode-marker regexes (`markerRegexes`), bypass Phase 1 entirely (`skipPhase1`), or inject an LLM prompt hint (`llmHint`). The combined snapshot then runs a multi-stage cascade: (0.5) **Content-ID short-circuit** — if `watchNext.contentId: tmdb:XXXX` is present and the TMDB ID is in the user's library, returns confidence=1.0 immediately; (1) **Phase 1 (cheap)** — candidate lines are sorted per profile's `preferredSourceTags`, then each is scored against the local show cache with the existing Levenshtein fuzzy matcher; episode markers are extracted using profile-specific regexes first then the default `S##E##` fallback; skipped entirely when `profile.skipPhase1 == true`; (2) **LLM fallback** — on cache miss (< 0.70), call `TitleExtractor.extract(snapshot)` which the TV binds to `PhoneTitleExtractionClient` (ships the snapshot + a top-50 library-hint list to the best phone's `POST /scrobble/extract`, dedups by `packageName:title` so the 30 s poll cycle can't stack duplicate inferences, 90 s client / 75 s server timeout to absorb cold LiteRT-LM inference); `LlmTitleExtractor.buildPrompt()` appends the profile's `llmHint` as `"App-specific note: ..."` when present; (3) **TMDB fallback** — search TMDB with whichever normalized title survived. Auto-scrobbles if confidence ≥ 95%, shows overlay confirmation between 70–95%, ignores below 70% (all thresholds live in `core/scrobbler/ScrobbleTuning.kt` — `ScrobbleTuning.DEFAULT` holds the production values documented here; tests may inject a custom instance). When a scrobble event occurs, `TvScrobbleDispatcher` calls `POST /scrobble/{start|pause|stop}` on **every** connected phone in parallel via `PhoneDiscoveryManager` + `PhoneApiClientFactory` — each phone records the episode on its own user's Trakt account using its own stored credentials. A failure for one phone does not block the others. The TV never calls the Trakt API directly for any operation. Progress is derived from `PlaybackState.position` and `MediaMetadata.METADATA_KEY_DURATION`; if unavailable, start/pause fall back to 0/50 and stop is skipped to avoid Trakt marking partially-watched episodes as watched (Trakt treats `progress >= 80` on `/scrobble/stop` as watched).
...
- **TV ShowDetail next-episode image and title (#366, fixed #498):** `ShowDetailViewModel.loadNextEpisode(EnrichedShowEntry)` calls TMDB `GET /tv/{id}/season/{s}/episode/{e}` (using `tmdbApiKey` from the best phone's `/capability`) to fetch the next unwatched episode's `still_path` and `name`. Episode numbers come from `ShowProgressCalculator.nextUnwatchedEpisodeNumbers()` (user-progress-first): (1) not started → S01E01; (2) behind on aired episodes → advance one episode past the highest watched, season-aware (crosses season boundaries at finales); (3) caught up and `TmdbProgressHint.nextAired` is a regular season → use nextAired; (4) caught up, no scheduled episode → naive +1 (callers handle 404). `loadDeepLinks` and `onProviderSelected` in `ShowDetailViewModel` also use `nextUnwatchedEpisodeNumbers` so the still image, episode title, JustWatch deep links, and Phase-0 scrobble hint all point to the same episode. The scrobbler fallback (`MediaSessionScrobbler`) continues to use `nextEpisodeNumbers()` (nextAired-first) which is the correct guess for what is currently playing on a live-airing show. The TV `ShowDetailScreen` left panel shows the fetched still image (falling back to series poster, then a plain surface); the right panel shows the actual episode title as the primary line and the `SxxExx` code as a secondary secondary line. State is exposed as `NextEpisodeUiState` (`isLoading`, `stillUrl`, `episodeName`, `episodeCode`). The nav graph (`TvNavGraph`) now passes `EnrichedShowEntry` instead of `TraktWatchedEntry` so TMDB hint data is available throughout the detail→recap flow.
- **TV ShowDetail "Available on" providers (#354/#355/#356/#367):** `ShowDetailViewModel.loadProviders(EnrichedShowEntry)` calls `WatchProvidersRepository.getResolvedProviders(tmdbId, countryCode, apiKey, showNonInstalled)`. The repository fetches TMDB `GET /tv/{id}/watch/providers`, merges flatrate/ads/free results, caches 24 h in-memory (keyed by `tmdbId:countryCode`), then orders the list as: **last-used** (from `LastUsedProviderRepository`) → **installed** (from `InstalledAppsProbe`) → **not-installed** (only if `showNonInstalledProviders` setting is on). State is exposed as `ProviderListUiState` (`Loading`, `Success(providers)`, `Empty(tmdbPageUrl)`, `Error`). `ShowDetailScreen` renders a `LazyRow` of `ProviderChip` cards; the first entry (last-used) gets a primary-color border and a "Last used" badge. All known package names are declared in a `<queries>` block in `app-tv/AndroidManifest.xml`. The manual streaming-service settings screen (`StreamingSettingsScreen` / `StreamingSettingsViewModel`) has been removed; the `StreamingPreferencesRepository` no longer stores subscribed-service lists.
- **JustWatch per-episode deep links (#476):** `ShowDetailViewModel.loadDeepLinks(EnrichedShowEntry)` is called once providers reach `ProviderListUiState.Success`. For each provider, a `viewModelScope.async` resolves the deep link via `JustWatchDeepLinkRepository.resolveDeepLink(tmdbShowId, season, episode, providerId, countryCode, showTitle, showYear)`. In-flight dedup at the ViewModel level prevents duplicate concurrent Deferred jobs for the same key. State is exposed as `deepLinks: StateFlow<Map<Int, DeepLinkState>>` where `DeepLinkState` is `Loading | Available(url) | Unavailable`. Tapping a chip calls `onProviderSelected(provider, enriched)` which returns the JustWatch URL when `Available`, falls back to `provider.tmdbPageUrl`, and records the choice in `LastUsedProviderRepository`. `resolveTopProviderDeepLink()` returns the JustWatch URL for the first provider for the quick-launch "Watch Now" button. A `JustWatchAttributionBadge` composable is shown when any link is `Available`. The `JustWatchDeepLinkRepository` resolves via a 4-step cascade: (1) episode Room cache, (2) live JustWatch GraphQL call (`SEARCH_QUERY` → `SEASONS_QUERY` → `EPISODES_QUERY`), (3) show-level Room cache, (4) show-level JustWatch call. Positive hits cached permanently; negatives expire after 30 days. Batch dedup: Mutex-protected map keyed by `(tmdbShowId, season, episode, countryCode)` prevents duplicate API calls across concurrent coroutines. `JustWatchPackageMap` translates JustWatch `technicalName` strings (e.g. `nfx`, `prv`, `dnp`) to TMDB `provider_id` integers. `ProviderCatalog` now only maps `provider_id → packageName` — deep-link templates are gone. The TV `TvDiagnosticsScreen` shows a "Streaming Links" section with cached count, negative count, last-fetch age, and a "Clear cache" button backed by `TvDiagnosticsViewModel.clearJustWatchCache()`.
- **Diagnostics view:** Settings → Diagnostics on both apps renders live phone↔TV connection health from the existing shared singletons (`CompanionStateManager` on phone, `PhoneDiscoveryManager` on TV) — Wi-Fi / IPv4 / service running / HTTP bind / BLE advertiser state on the phone; discovery active / heartbeat age / BLE scan state / per-phone score + failCount + RSSI on the TV (#331). The TV Diagnostics screen also includes a **Watch Next provider** section (count of apps publishing fresh rows; when `READ_TV_LISTINGS` is denied the row turns red and an "Open app permissions" CTA opens the system app-details settings via `Settings.ACTION_APPLICATION_DETAILS_SETTINGS`. The permission is requested at runtime from `TvMainActivity.maybeRequestReadTvListingsPermission()` because not every Google TV / 3rd-party Android TV firmware auto-grants `signature|privileged|appop` permissions. **Crucially, `WatchNextMetadataSource` queries the WatchNext provider with `selection = null, selectionArgs = null` and filters by package-name + freshness in app code** — AOSP `TvProvider.createSqlParams(...)` throws `SecurityException("Selection not allowed for content://android.media.tv/watch_next_program…")` for any non-empty selection unless the caller holds `com.android.providers.tv.permission.ACCESS_ALL_EPG_DATA` (signature|privileged, unattainable for normal apps), and `READ_TV_LISTINGS` does not exempt the caller. The canonical red/yellow/green signal is `ContextCompat.checkSelfPermission(READ_TV_LISTINGS)`, evaluated in `countPublishingApps()` before any provider round-trip. The legacy `@Volatile permissionDenied` flag is kept as a defensive backstop so a surprise SecurityException from a downstream OEM fork can't spam the log on every scrobble tick; it is cleared explicitly via `resetPermissionState()` (called from `TvDiagnosticsViewModel.refreshWatchNextStats()` on `ON_RESUME` and from `TvMainActivity` after a successful runtime grant), **and** auto-healed on each call by `isPermissionCurrentlyDenied()` which performs a live `ContextCompat.checkSelfPermission` check when the flag is set — this covers the case where the user grants the permission in system Settings without returning through the in-app dialog or the diagnostics screen (#494, #543)), a **Media notifications** section (count of packages with MediaStyle notifications tracked by `NotificationMetadataSource` in the last 10 min — green when ≥1 observed, yellow when connected but no data, red when the notification listener is not connected (#472)), and an **App profiles** section ("`N / M observed`" counter where N = packages with an `AppProfile` entry and M = total distinct packages seen in `getActiveSessions` since service start — sourced from `MediaSessionScrobbler.observedPackageStats()` (#473)). Status dots are color-coded (green/yellow/red) so users can tell "no phones in BLE range" apart from "`/capability` 500" (discovered but broken). On the **phone only**, a "Share diagnostics" button funnels through `DiagnosticShare.launchShare()` so the `DiagnosticLog` breadcrumb snapshot + any pending crash reports can be exported via the system share sheet (#338). The TV screen is view-only — TV share was removed because the Android TV system share sheet is effectively unusable with a D-pad. For the snapshot to be actionable, connectivity subsystems on the phone (`CompanionService`, `CompanionHttpServer` request log, `CompanionBleAdvertiser`, `WifiStateProvider`, `HomeViewModel` toggle) emit `DiagnosticLog.event(...)` breadcrumbs — without those the ring only carries auth/settings/app-lifecycle traces. Available in release builds; no new build variant.
