...
The phone acts as an NSD/mDNS server on port 8765. The TV discovers phone(s) on the local network and calls the phone's HTTP API. A parallel **BLE fallback** discovery channel (`CompanionBleAdvertiser` on the phone, `PhoneBleScanner` on the TV) bypasses the Wi-Fi data plane for networks where mDNS is blocked (AP/client isolation, VLAN-segmented mesh Wi-Fi, aggressive multicast filtering). BLE advertises the same phone LAN endpoint as NSD; the TV then connects via plain HTTP once it has the `(ip, port)` pair. Both channels converge on the same capability-fetch pipeline and dedup by `baseUrl`.
