...
│       └── (service/)  CompanionService, CompanionStateManager (foreground BLE advertiser + HTTP server + shared state)
...
- **Presence heartbeat (TV):** `PhoneDiscoveryManager` runs a heartbeat every 60 s, re-fetching `/capability` for each phone. 3 consecutive failures → phone removed from the discovered list. `MediaSessionScrobbler` skips phones with stale presence (> 2 min) before scrobbling. A `ConnectivityManager.NetworkCallback` restarts the BLE scanner when Wi-Fi returns, in case the BLE stack was silently reset. Because BLE range can exceed LAN reach, a phone that's out of Wi-Fi range but still within BLE range will fail `/capability` and be evicted after `MAX_CONSECUTIVE_FAILURES = 3` heartbeat misses (~3 min).
- **Presence timeout (phone):** If no TV polls `/capability` for 5 minutes, the companion service auto-deactivates.
- **Wi-Fi handoff (phone):** `CompanionService` registers a `ConnectivityManager.NetworkCallback` for Wi-Fi. On network loss the BLE advertiser is stopped immediately and a 3 s grace timer runs — if Wi-Fi returns within that window `onAvailable` restarts the advertiser with the new IPv4 embedded in the payload; otherwise the service self-stops and clears `companionEnabled` so the FG notification is dismissed (#278). `onStartCommand` is idempotent against duplicate starts.
- **BLE discovery (sole channel):** `CompanionBleAdvertiser` broadcasts a 9-byte service-data payload (IPv4, port, `modelQuality`, `llmBackend` ordinal) under a custom UUID defined in `core/discovery/BleDiscoveryContract.kt`. The advertisement carries only a Service Data 128-bit AD field (no separate Complete-List-of-UUIDs AD) to stay under the 31-byte legacy envelope — Android 16 / Nothing reject the old 48-byte payload with `DATA_TOO_LARGE` (#345). The advertiser is pinned to `ADVERTISE_MODE_BALANCED` (~250 ms) + `ADVERTISE_TX_POWER_MEDIUM` (~10 m, couch-to-TV range) and is non-connectable — the TV never opens a GATT connection. `PhoneBleScanner` runs continuously in `SCAN_MODE_BALANCED` whenever discovery is enabled, filters with `setServiceData(uuid, schemaVersion, 0xFF mask)` (also rejects adverts from a future incompatible schema), surfaces `result.rssi` alongside the decoded payload, and feeds the `/capability` fetch pipeline, deduped by `baseUrl`. Bluetooth-off / permission-denied / BLE-unsupported hardware degrade silently — the pair simply cannot connect until BLE is available again. Permissions: `BLUETOOTH_ADVERTISE` (phone) is requested from HomeScreen when the "I am watching TV" toggle flips on, and `BLUETOOTH_SCAN` (`neverForLocation`) is requested by `TvMainActivity.onCreate`. RSSI is plumbed into `DiscoveredPhone.rssi` and shown read-only in TV Diagnostics — no automatic filtering yet (tracked as a follow-up once real-world distributions are known).
